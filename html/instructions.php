	...
	if (allow == 1){
		...
		if(pos_x >= 139 && pos_x <= 285)
		{
			if(pos_y >= 142 && pos_y <= 204)
			{
				//Tophat
				$( "#dialog" ).dialog( "open" );
				$("#dialog p").text("Using a reference FASTA file, Tophat maps reads to the genome. More formally, TopHat is a fast splice junction mapper for RNA-Seq reads. It aligns RNA-Seq reads to mammalian-sized genomes using the ultra high-throughput short read aligner Bowtie, and then analyzes the mapping results to identify splice junctions between exons. ");
			...
			else if (pos_y >= 342 && pos_y <= 403)
			{
				//CUFFLINKS
				$( "#dialog" ).dialog( "open" );
				$("#dialog p").text("Cufflinks assembles mapped reads into transcripts with relative expression values for each isoform and gene.");
